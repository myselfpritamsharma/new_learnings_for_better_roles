...
- **Competitive programming track (persistent):** The user is preparing against competitive programmers. Maintain a CSES-based track from `https://cses.fi/problemset/` alongside interview DSA. Add CSES items in grouped progression (introductory, sorting/searching, DP, graphs, range queries, trees, mathematics/strings, then advanced topics), keep them unsolved until the user attempts them, and use C#/.NET with fast I/O, constraints-first reasoning, proof/invariant discussion, complexity, edge cases, and post-submit failure analysis.
...
- **Language switch (persistent, effective now):** The user has moved off Python and is standardizing on **C#/.NET** for LeetCode, LLD, and HLD work, matching their production stack. Study `coding-mentor/Coding-Mentor-Dashboard.html` → **C# Guide** tab first (syntax, collections/Big-O, LINQ, OOP idioms, concurrency primitives, DSA pattern templates) before resuming new problem-solving. Once that foundation is solid, write all new DSA/LLD solutions, hints, and code reviews in C# by default — not Python. Previous Python solution files and solved-answer blocks were removed for a clean restart; do not recreate them unless explicitly requested.
