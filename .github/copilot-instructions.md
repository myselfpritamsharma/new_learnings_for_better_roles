...
- **Current Capacity:** The user can study 2-3 hours/day. Use this as the default planning budget: DSA 60-75 min, SQL/DBMS/Core CS 35-45 min, LLD/HLD/project 45-60 min, review 10-15 min.
...
- **Dashboard structure rule (persistent, do not ask again):** All day/week numbering, dates, scheduling, and "what to do today/this week" routing belongs ONLY in the Tasks tab of `coding-mentor/Coding-Mentor-Dashboard.html`. The DSA, LLD, and HLD tabs must contain ONLY pure, timeless content: questions, concepts, theory, solutions, hints, things to remember, edge cases, and deep-dive explanations. Never add "Day N" or "Week N" labels to section headings inside the DSA/LLD/HLD tabs — name sections after the concept/problem itself (e.g. "Rate Limiter", not "Day 4 Theory: Rate Limiter"). If a Tasks-tab entry needs to point into DSA/LLD/HLD content, link to it by its concept name, not by day.
- **Dashboard checklist data contract (persistent):** `coding-mentor/checklist-items.json` is the internal source of truth for interview DSA, LLD, and HLD definitions. The dashboard also loads the complete current CSES catalog from `https://cses.fi/problemset/` at startup, currently 400 official tasks, and merges it into the same DSA topic groups. When adding, removing, renaming, or regrouping checklist items, update the JSON/loader contract first and keep the dashboard renderer/fallback markup synchronized. Do not hardcode a new checklist item only in HTML.
- Checklist JSON is an internal implementation detail, not a user export/import feature. Never add export buttons, import controls, file-upload controls, or export/import handlers for checklist definitions or progress. Checklist progress remains encrypted in browser `localStorage`; preserve the existing status cycle and reset behavior unless explicitly asked to change them.
- The dashboard should load `checklist-items.json` and the complete official CSES catalog at startup, then render one unified DSA checklist by topic. Keep a graceful embedded-markup fallback for direct-file opening, and validate that JSON keys, live CSES task IDs, and fallback `data-key` values match after checklist changes. Use the existing JSON shape for local definitions: `{ "version": 1, "groups": [{ "title": "...", "items": ["stable-key"] }] }`.
- **Competitive programming track (persistent):** The user is preparing against competitive programmers. Maintain a CSES-based track from `https://cses.fi/problemset/` alongside interview DSA. Add CSES items in grouped progression (introductory, sorting/searching, DP, graphs, range queries, trees, mathematics/strings, then advanced topics), keep them unsolved until the user attempts them, and use Java with fast I/O, constraints-first reasoning, proof/invariant discussion, complexity, edge cases, and post-submit failure analysis.
- **Unified DSA bank (persistent):** Keep interview DSA and CSES competitive-programming questions in one DSA tab/checklist, grouped by topic. Every coding question must display an Easy, Medium, or Hard label. End each DSA/CSES topic group with a clearly labeled mixed-topic checkpoint that combines multiple patterns; do not hide mixed questions in a separate ungrouped list.
- **Advanced fusion checklist (persistent):** Keep the eight Advanced Fusion Patterns prompts inside the unified DSA checklist as Hard questions: HashMap + doubly linked list + min-heap, segment tree/Fenwick + HashMap, monotonic deque + sliding window + HashMap, Trie + backtracking + matrix DFS, modified Dijkstra + HashMap + DP, DSU + cycle detection + Kruskal, sliding window + dual heaps + HashMap, and sweep line + priority queue.
- **Clean restart rule (persistent):** The dashboard is reset to Day 1. Keep DSA, LLD, and HLD tabs focused on checklist questions only in the active study view; old theory/answer blocks, solved-problem tables, solution links, and historical answer sections must stay removed or hidden unless the user explicitly requests a new solution after attempting a question.

### 5. Language Positioning & Senior-Level Depth
- **Language strategy (persistent, effective now):** Java is the immediate default for all new DSA, CSES, LeetCode, LLD, mock-interview, and interview-coding work because it aligns with GCC and investment-banking backend roles. Use Java/Spring Boot examples for interview and finance-market alignment. The renamed **.NET Production Guide** is for C#/.NET depth: CLR, GC, allocation, async/await, ASP.NET Core, concurrency, SQL Server, observability, and production architecture. Learn system design, SQL/DBMS, Kafka, Redis, cloud/DevOps, security, and observability once as shared foundations, then apply them in Java and .NET. Angular is maintenance-only and must remain below 10% of preparation time. Previous Python solution files and solved-answer blocks were removed; do not recreate them unless explicitly requested.
- **Java curriculum order (persistent):** Build Java only to the level needed for DSA, LLD, and backend interviews: core syntax/OOP, collections and Big-O, strings/arrays, generics, functional Java, exception handling, JVM/memory, concurrency, I/O/networking basics, modern Java, then Spring Boot/JPA/Hibernate/Kafka/Redis. Prioritize collections, OOP, generics, concurrency, JVM basics, Spring Boot, SQL, Kafka, Redis, and distributed systems over obscure Java libraries.
- **Three-layer routing (persistent):** DSA = Java language → collections → patterns → algorithms → timed problems. LLD = Java OOP → SOLID → design patterns → concurrency → machine coding. HLD = Java/Spring Boot → APIs → SQL → Kafka → Redis → cloud → distributed systems → complete designs. C#/.NET is used to demonstrate production engineering depth, not to duplicate the shared curriculum.
- At 6-7 YOE, senior GCC interviews probe multithreading/concurrency in depth, not just DSA patterns: memory visibility (volatile, memory barriers), locks vs lock-free (CAS, atomics), thread pools and rejection policies, producer-consumer with blocking queues, deadlock/livelock/starvation causes and prevention (lock ordering, tryLock with timeout), and coordination primitives (CountDownLatch/CountdownEvent, CyclicBarrier/Barrier, Semaphore).
- Backend/HLD prep must also cover: event-driven vs request-response architecture (Kafka/RabbitMQ delivery semantics, ordering, backpressure), low-latency tuning (GC/Stop-the-World pauses, reduced allocations, ring buffers like LMAX Disruptor), and database concurrency (ACID, isolation levels, optimistic vs pessimistic locking, deadlock handling).
- Classic senior LLD prompts to keep in rotation: Thread-Safe In-Memory Order Book, Rate Limiter/Throttler, and a High-Performance Caching Layer with concurrent reads and single-writer safety (Reader-Writer lock).
- GCC/Citadel-tier rounds increasingly fuse 2-3 data structures into one problem (e.g. HashMap + Doubly Linked List + Min-Heap, Segment Tree + HashMap, Trie + Backtracking + Matrix DFS, DSU + cycle detection, Sweep-Line + Priority Queue). These live in the DSA tab's "Advanced Fusion Patterns" section as stretch goals for later phases — do not front-load them before the base patterns (weeks 1-8) are solid.
