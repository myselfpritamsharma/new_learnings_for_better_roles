...
- **Dashboard checklist data contract (persistent):** `coding-mentor/checklist-items.json` is the internal source of truth for all DSA, LLD, and HLD checklist definitions. It currently contains 11 groups and 220 stable item keys. When adding, removing, renaming, or regrouping checklist items, update this JSON first and keep the dashboard renderer/fallback markup synchronized. Do not hardcode a new checklist item only in HTML.
...
- The dashboard should load `checklist-items.json` at startup and render checklist groups/items from it. Keep a graceful embedded-markup fallback for direct-file opening, and validate that JSON keys and fallback `data-key` values match after checklist changes. Use the existing JSON shape: `{ "version": 1, "groups": [{ "title": "...", "items": ["stable-key"] }] }`.
